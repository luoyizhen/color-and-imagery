...
function createBubbleChart() {
    ...
    function getFillColorScale() {
        ...
        for (var i=0; i<color_groupsKeys.length; i++) {
            ...
            //console.log(color_groupsValues)
        }
    ...
    function createNodes(rawData) {
        ...
        var myNodes = rawData.map(function (data_row) {
            radius = +data_row[BUBBLE_PARAMETERS.radius_field[0]]
            //console.log(radius)
            node = {
                id: data_row.id,
                scaled_radius: radiusScale(radius),
                color: +data_row[BUBBLE_PARAMETERS.color],
...
/*         .attr("cx",function(){
        ...
        //.attr("dy", ".3em")
        .style("text-anchor", "middle")
        .style("font-style","calibri")
        .style("font-weight","bold")
        .style("fill","black")
        //.style("pointer-events", "none")
        .text(function(d){
            console.log(d.actual_radius)
            return d.actual_radius;
        })
    ...
    var bubbleChart = function bubbleChart(parentDOMElement, rawData) {
        ...
        //console.log(BUBBLE_PARAMETERS.radius_field[1])
        ...
        radiusScale = d3.scalePow()
            ...
            .domain([0, 1000]);   // Domain between 0 and the largest bubble radius
    ...
    bubbleChart.switchMode = function (buttonID) {
        ...
        if (currentMode.type == "grid" && currentMode.size > 1) {
            ...
            //rawData.map(function (data_row) {
            //    radius = +data_row[BUBBLE_PARAMETERS.radius_field[0]]
            //var text = currentMode.
            //radius = 500
            //+data_row[BUBBLE_PARAMETERS.radius_field[buttonId]]
...
/*             var myNodes = rawData.map(function (data_row) {
        ...
        if (currentMode.type == "scatterplot") {
            //console.log(dataExtents[currentMode.xDataField][0])
        ...
        if (currentMode.type == "scatterplot") {
            targetFunction = function (d) {
                return { 
                    x: xScale(d[currentMode.xDataField]*0.8),
                    y: yScale(d[currentMode.yDataField]*0.8)
                };
            };
        }
        if (currentMode.type == "map") {
...
function ViewMode(button_id, width, height) {
    ...
    if (this.type == "grid") {
        ...
        //this.radius = curMode.radius_field;
